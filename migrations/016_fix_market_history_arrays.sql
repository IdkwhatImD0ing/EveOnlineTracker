-- Migration 016: Fix market history arrays function to remove minimum data point constraint
-- The HAVING COUNT(*) >= 30 constraint was preventing results from showing for
-- shorter time periods (7D, 30D) where fewer than 30 data points exist.
...
BEGIN
  ...
  aggregated AS (
    ...
    GROUP BY oh.type_id
    -- Removed: HAVING COUNT(*) >= 30 constraint that prevented short time periods from working
  )
  SELECT 
    a.type_id,
