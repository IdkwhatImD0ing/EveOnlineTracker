...
import { VelocitySubtab } from "./velocity-subtab"

interface DashboardTabProps {
  // Capital data
  capitalData: CapitalEfficiencyResponse | null
  capitalLoading: boolean
  capitalError: string | null
  capitalProgress: ProgressState | null
  onCapitalRefresh: () => void
  
  // Velocity data
  velocityData: TradingVelocityResponse | null
...
export function DashboardTab({
  ...
  const characters = useMemo(() => {
    if (!capitalData?.summary?.byCharacter) return []
    return capitalData.summary.byCharacter
  }, [capitalData?.summary?.byCharacter])

  // Filter orders by selected location and character
  const filteredOrders = useMemo(() => {
    if (!capitalData?.orders) return []
    let orders = capitalData.orders
    ...
    return orders
  }, [capitalData?.orders, selectedLocation, selectedCharacter])

  // Recalculate summary metrics for filtered orders
  const filteredSummary = useMemo(() => {
    if (!capitalData?.summary) return null
    if (selectedLocation === "all" && selectedCharacter === "all") return capitalData.summary
  ...
  }, [capitalData?.summary, filteredOrders, selectedLocation, selectedCharacter])
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <Tabs value={activeSubTab} onValueChange={(v: string) => setActiveSubTab(v as "capital" | "velocity")} className="space-y-4 md:space-y-6">
        <TabsList className="grid w-full max-w-md grid-cols-2 h-auto">
          <TabsTrigger value="capital" className="gap-1.5 md:gap-2 text-xs md:text-sm py-2.5">
          ...
          <TabsTrigger value="velocity" className="gap-1.5 md:gap-2 text-xs md:text-sm py-2.5">
            <Zap className="size-3.5 md:size-4" />
            Velocity
            {velocityData && velocityData.trend.direction === 'up' && (
              <Badge variant="secondary" className="ml-1 px-1.5 py-0 text-xs bg-emerald-500/20 text-emerald-600">
                ↑
              </Badge>
            )}
          </TabsTrigger>
        ...
        <TabsContent value="capital">
          <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-2">
                      ...
                      <Select value={selectedCharacter} onValueChange={setSelectedCharacter}>
                        ...
                        <SelectContent>
                          <SelectItem value="all">
                            All Characters ({capitalData?.orders.length})
                          </SelectItem>
                    ...
                    {showLocationFilter && (
                      <Select value={selectedLocation} onValueChange={setSelectedLocation}>
                        ...
                        <SelectContent>
                          <SelectItem value="all">
                            All Systems ({capitalData?.orders.length})
                          </SelectItem>
                          {locations.map((loc) => (
                    ...
                    <CollapsibleContent>
                      <div className="text-sm text-muted-foreground bg-muted/50 rounded-lg p-4 mt-2">
                        <ul className="space-y-1 text-xs">
                          <li>• <strong>Est. Daily Sales</strong> = Regional Volume × {capitalData.config.hubFactor * 100}% (hub factor)</li>
            ...
            {capitalLoading && capitalProgress && (
              <Card>
                <CardContent className="py-8">
                  <ProgressBar progress={capitalProgress} />
                </CardContent>
              </Card>
            )}
            ...
            {capitalData && filteredSummary && (
              <>
                <div className="grid gap-4 md:grid-cols-4">
                  ...
                  <Card>
                    <CardContent className="p-4">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Based on {capitalData.config.hubFactor * 100}% of regional volume
                      </p>
                    </CardContent>
                  </Card>
                  <Card>
                ...
                {capitalData.summary.byCharacter && capitalData.summary.byCharacter.length > 0 && (
                  <Card>
                      ...
                      <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <div className="flex-shrink-0 flex flex-col items-center gap-2">
                          <div
                            ...
                            style={{
                              background: generatePieGradient(capitalData.summary.byCharacter),
                            }}
                          />
                          <p className="text-xs text-muted-foreground">
                            {capitalData.summary.byCharacter.length} character{capitalData.summary.byCharacter.length !== 1 ? 's' : ''}
                          </p>
                        </div>
                        
                        {/* Character List */}
                        <div className="flex-1 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                          {capitalData.summary.byCharacter.map((char, index) => (
                            <div
        ...
        <TabsContent value="velocity">
          <VelocitySubtab
            ...
            onPeriodChange={onVelocityPeriodChange}
            onRefresh={onVelocityRefresh}
          />
        </TabsContent>
      </Tabs>
    </div>
