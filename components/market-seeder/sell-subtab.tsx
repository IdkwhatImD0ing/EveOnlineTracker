...
// Low margin threshold - items below this margin are flagged
const LOW_MARGIN_THRESHOLD = 20 // 20%
...
function calculateMargin(sellPrice: number, jitaPrice: number): number {
  ...
  return ((sellPrice - jitaPrice) / jitaPrice) * 100
}
...
export function SellSubtab({
  ...
  // Sort all items by ISK/day (highest first) and filter by character if selected
  const sortedItems = useMemo(() => {
    ...
    return data.items
      ...
      .sort((a, b) => b.isk_per_day - a.isk_per_day)
  }, [data, selectedCharacter])

  // Calculate sidebar items
  const sidebarItems = useMemo(() => {
    if (!data) return { existingOrders: [], lowMargin: [] }

    // Get items with existing orders
    ...
    // Get low margin items (margin < 20% from Jita)
    const lowMargin = data.items
      .filter(item => {
      ...
      .sort((a, b) => {
        ...
        const marginA = calculateMargin(a.sell_price, a.jita_price)
        const marginB = calculateMargin(b.sell_price, b.jita_price)
...
Zeugma Integrated Analyzer		Data Miners		Medium	5 m3	530,611,313.87 ISK"
      ...
      {data && sortedItems.length > 0 && (
        <div className="flex items-center justify-between">
          <span className="text-sm text-muted-foreground">
            {sortedItems.length} items sorted by ISK/day
          </span>
          <Button
            variant="outline"
            size="sm"
          ...
          >
            {copyAllSuccess ? (
              ...
              <>
                ...
                Copy All Prices ({sortedItems.length})
              </>
            )}
          </Button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg flex items-center gap-2">
                  <ShoppingCart className="size-5" />
                  Sell Orders ({sortedItems.length} items)
                </CardTitle>
                <CardDescription>
                ...
                <div className="space-y-3">
                  {sortedItems.map((item) => {
                    ...
                    return (
                      <div
                        key={item.type_id}
                        className={`p-4 rounded-lg border transition-colors ${isLowMargin
        ...
        <div className="lg:sticky lg:top-4 lg:self-start space-y-4">
          {/* Has Existing Orders */}
          {data && sidebarItems.existingOrders.length > 0 && (
            <Card className="border-blue-500/30">
              <CardHeader className="py-4">
                <CardTitle className="text-base flex items-center gap-2">
                  <ShoppingCart className="size-4 text-blue-500" />
                  Has Existing Orders ({sidebarItems.existingOrders.length})
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-0">
                <div className="space-y-1 max-h-48 overflow-y-auto">
                  {sidebarItems.existingOrders.map((item) => (
          ...
          {/* Low Margin Warning */}
          {data && sidebarItems.lowMargin.length > 0 && (
            <Card className="border-orange-500/30">
              <CardHeader className="py-4">
                <CardTitle className="text-base flex items-center gap-2">
                  <TrendingDown className="size-4 text-orange-500" />
                  Low Margin ({sidebarItems.lowMargin.length})
                </CardTitle>
                <CardDescription className="text-xs">
                  Items with less than {LOW_MARGIN_THRESHOLD}% margin from Jita sell
                </CardDescription>
              </CardHeader>
              <CardContent className="pt-0">
                <div className="space-y-1 max-h-64 overflow-y-auto">
                  {sidebarItems.lowMargin.map((item) => {
                    const margin = calculateMargin(item.sell_price, item.jita_price)
                    return (
                      <div key={item.type_id} className="flex items-center gap-2 text-sm py-1.5 px-2 bg-orange-500/5 rounded min-w-0">
                        <EveItemIcon typeId={item.type_id} size={32} className="size-5 shrink-0" />
                        <div className="flex-1 min-w-0">
                          <span className="truncate text-xs block">{item.type_name}</span>
                          <span className="text-[10px] text-muted-foreground">
                            Jita: {item.jita_price_formatted}
                          </span>
                        </div>
                        <div className="text-right shrink-0">
                          <Badge
                            variant="outline"
                            className={`text-[10px] px-1 ${margin < 10
          ...
          {/* All clear state */}
          {data && sidebarItems.existingOrders.length === 0 && sidebarItems.lowMargin.length === 0 && (
            <Card className="border-dashed border-muted">
              <CardContent className="py-8">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">All items have good margins</p>
                </div>
              </CardContent>
            </Card>
          )}
