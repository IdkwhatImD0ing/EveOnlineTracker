 ...
 * - Just item name (quantity defaults to 1)
 */
function parseItemsLocally(input: string): ParsedItem[] {
  ...
  for (const line of lines) {
    ...
    // Format: Item Name \t Quantity \t Volume \t Buy \t Sell \t TotalBuy \t TotalSell
    ...
    // Fallback: treat entire line as item name with quantity 1
    // But skip lines that look like numbers only
    if (!/^[\d\s,.]+$/.test(trimmed)) {
      items.push({
        itemName: trimmed,
        typeId: 0,
